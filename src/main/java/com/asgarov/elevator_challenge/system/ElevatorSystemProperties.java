...
public class ElevatorSystemProperties {
    ...
    static {
        NUMBER_OF_ELEVATORS = Integer.parseInt(getProperties().getProperty("system.elevators.amount"));
        DELAY_BETWEEN_NEW_REQUEST_CHECKS_IN_MS = Integer.parseInt(getProperties().getProperty("system.request.checks.delay"));
        SHUTDOWN_ATTEMPTS_DELAY = Integer.parseInt(getProperties().getProperty("system.shutdown.attempts.delay"));
    }
}
